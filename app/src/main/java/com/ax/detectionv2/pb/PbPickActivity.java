...
import com.ax.detectionv2.tflite.FaceView;
import com.ax.detectionv2.tflite.TFLiteObjectDetectionAPIModel;
...
public class PbPickActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_pb_pick);
...
//                    detectImageView(bitmap);
    ...
    private void detectFace(Bitmap bitmap) {
        ...
        List<Classifier.Recognition> faces2 = new ArrayList<>();
        for(final Classifier.Recognition faceResult : faces){
            if(faceResult.getTitle().equals("person")){
                faces2.add(faceResult);
            }
        }
        faceView.setRotate(rotate);
        result_text.setText("Kết quả: " + time + "ms; Số lượng khuôn mặt phát hiện:" + faces2.size());

        if (count(faces2) == 0) {
            Toast.makeText(this, "Không tìm thấy!!!", Toast.LENGTH_SHORT).show();
        } else {
            for (final Classifier.Recognition faceResult : faces2) {
    ...
    private void resetData() {
        if (imagePreviewAdapter == null) {
            ...
            imagePreviewAdapter = new ImagePreviewAdapter(PbPickActivity.this, facesBitmap, new ImagePreviewAdapter.ViewHolder.OnItemClickListener() {
                @Override
